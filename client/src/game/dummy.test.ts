...
test('reversal blocks while it can act and works the motion while it cannot', () => {
  const d = createDummy()
  d.setMode('reversal')
  expect(d.poll(seen({actionable: true, attacking: true}))).toBe(IN_RIGHT)
  ...
  expect(d.poll(stunned)).toBe(IN_LEFT)
  expect(d.poll(stunned)).toBe(IN_DOWN)
  expect(d.poll(stunned)).toBe(IN_LEFT | IN_LP)
})
